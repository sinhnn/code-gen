const { Language, PropertyInfo, ClassSpec, Line, LineWithChildren, WrapWithChildren } = require("../spec.js");
...
class Javascript extends Language {
    ...
    to__constructor(properties, level = 1) {
        // const lines = [];
        const b = new WrapWithChildren(level, 
            `constructor (${properties.map(p => p.name).join(", ")}) {`,
            "}", 
            properties.map(p => `this._${p.name} = ${p.name};`)
        );
        b.add("this.__modified = []; // list of modified properties");
        return [b];
    }
    ...
    to__setter(property, level = 2) {
        ...
        lines.push(new WrapWithChildren(level, 
